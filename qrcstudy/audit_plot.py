...
def render(root='.'):
    ...
    root=Path(root);snapshot=root/'data/snapshots/extended-2026-09-24-v2';out=root/'results/extended-study-2026-09-24';out.mkdir(parents=True,exist_ok=True)
    ...
    axes[1].plot(targets.index,targets.difference*1e6,color='#79529c');axes[1].set_ylabel('Log-RV difference × 1,000,000');axes[1].set_title('Extended target minus independently reconstructed price target')
