...
def summarize(root='.'):
    root=Path(root);out=root/'results/colin-study-2026-09-24';out.mkdir(parents=True,exist_ok=True)
    from .audit_plot import render
    render(root)
    snapshot=root/'data/snapshots/colin-2026-09-24-v2';audit=json.loads((snapshot/'manifest.json').read_text());receipts={};headline=[];coverage=[]
    for label,folder in [('Colin paper features','colin-2026-09-24'),('Modern price features','modern-2026-09-24')]:
