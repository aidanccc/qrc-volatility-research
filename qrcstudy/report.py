...
def load_validated(folder):
    ...
    if not data.exists():
        # Published reports remain portable while the recorded absolute path
        # preserves the original execution location.
        data=Path(__file__).resolve().parents[1]/"data"/"snapshots"/data.parent.name/data.name
